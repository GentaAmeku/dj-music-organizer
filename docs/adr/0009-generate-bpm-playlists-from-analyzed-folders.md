# ADR 0009: BPMプレイリストはコレクション別に生成する
...
ただし、全タイトルを横断した `playlists/BPM/{BPMRange}.m3u8` だけでは、DJMAX、RagnarokOnline、別ゲームOSTなどが混ざってしまう。ユーザーがタイトル単位で探したい場合、この構造は使いにくい。

`00_Inbox/{Source}/...` の `{Source}` は `source_hint` としてログに残っている。これはタイトルやゲーム単位のコレクションIDとして使える。
...
BPMプレイリストはコレクション別を主系統として生成する。

出力先は以下とする。

```text
DJ Music/playlists/Collections/{source_hint}/{source_hint}_All.m3u8
DJ Music/playlists/Collections/{source_hint}/BPM/{BPMRange}.m3u8
```

全タイトル横断で見たい場合は、別系統として以下へ生成する。

```text
DJ Music/playlists/Global/BPM/{BPMRange}.m3u8
```
...
rekordboxに取り込むためのBPM別プレイリストを、タイトルやゲーム単位で分けられる。

`source_hint` が異なる曲は別の `Collections/{source_hint}` に入るため、DJMAXと別ゲームOSTが同じBPMプレイリストで混ざりにくくなる。
...
`source_hint` はInbox投入時のディレクトリ名に依存する。別タイトルの曲を正しく分けたい場合は、`00_Inbox/{Title}/` の段階でタイトル別に分ける必要がある。
